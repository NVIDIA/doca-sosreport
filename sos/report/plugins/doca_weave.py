...
class DocaWeave(Plugin, IndependentPlugin):
    ...
    def _collect_logs(self):
        # add_container_logs feeds the container name to the runtime, but
        # crictl logs resolves only IDs, so build the commands from IDs.
        # get_all also captures exited instances, the common case on failure.
        runtime = self._get_container_runtime()
        if runtime is None:
            return
        for name in WEAVE_CONTAINERS:
            for cid, cname in self.get_all_containers_by_regex(
                    name, get_all=True):
                self.add_cmd_output(
                    runtime.get_logs_command(cid),
                    suggest_filename=f"{cname}-{cid[:12]}.log",
                )
    ...
    def _describe_resources(self, list_cmd, list_key, get_cmd):
        ...
        if not isinstance(parsed, dict):
            self._log_error(f"Unexpected non-object JSON from {list_cmd}")
            return
